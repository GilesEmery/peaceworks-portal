...
export function buildPeaceWorksEmailHtml(input: PeaceWorksEmailTemplateInput) {
  ...
  const logoUrl = `${siteUrl}/icons/peaceworks-192.png`;
  ...
  return `<!doctype html><html lang="en"><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"></head><body style="margin:0;padding:0;background:#f3f1e9;color:#1f2922;font-family:Arial,Helvetica,sans-serif">${preview}<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;background:#f3f1e9"><tr><td align="center" style="padding:28px 12px"><table role="presentation" width="640" cellspacing="0" cellpadding="0" border="0" style="width:100%;max-width:640px"><tr><td align="center" style="padding:0 0 20px"><a href="${escapeHtml(siteUrl)}" style="text-decoration:none"><img src="${escapeHtml(logoUrl)}" width="72" height="72" alt="PeaceWorks" style="display:block;width:72px;height:72px;border:0;border-radius:18px"></a><div style="margin-top:10px;color:#355f40;font-size:13px;font-weight:700;letter-spacing:2px;text-transform:uppercase">PeaceWorks</div></td></tr><tr><td style="background:#ffffff;border:1px solid #e2ded2;border-radius:22px;padding:40px 42px"><h1 style="margin:0 0 12px;color:#1f2922;font-family:Georgia,'Times New Roman',serif;font-size:34px;line-height:1.18;font-weight:700">${escapeHtml(input.title)}</h1>${metadata ? `<p style="margin:0 0 28px;color:#69746c;font-size:14px;line-height:1.5">${escapeHtml(metadata)}</p>` : ""}${renderCommunicationBodyHtml(input.body)}<table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin-top:28px"><tr><td bgcolor="#355f40" style="border-radius:999px"><a href="${escapeHtml(cta.url)}" style="display:inline-block;padding:14px 24px;color:#ffffff;font-size:15px;font-weight:700;text-decoration:none;border-radius:999px">${escapeHtml(cta.label || "Visit PeaceWorks")}</a></td></tr></table></td></tr><tr><td align="center" style="padding:22px 18px 0;color:#6b746d;font-size:12px;line-height:1.6"><strong style="color:#355f40">PeaceWorks</strong><br>Make Peace Practical<br><a href="${escapeHtml(siteUrl)}" style="color:#526b57;text-decoration:underline">peaceworks.network</a><br><span style="color:#858c86">You received this message because of your PeaceWorks account or Circle participation.</span></td></tr></table></td></tr></table></body></html>`;
}
...
export function buildPeaceWorksEmailText(input: PeaceWorksEmailTemplateInput) {
  ...
  return [
    ...
    "PeaceWorks · Make Peace Practical",
    siteUrl,
